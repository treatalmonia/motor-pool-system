  ...
  <v-container fluid>
      ...
      <v-col>
        <div class="d-flex align-center justify-space-between flex-wrap ga-2">
          ...
          <div class="d-flex ga-2 flex-wrap align-center">
            ...
            <!-- AFTER — disable until record exists -->
            <v-btn
              color="success"
              variant="outlined"
              prepend-icon="mdi-pencil"
              :disabled="!signatories"
        ...
        <v-card rounded="lg" elevation="0" border>
          <v-card-text class="d-flex align-center ga-3">
            <v-avatar color="primary" variant="tonal" size="44"
              ><v-icon>mdi-format-list-bulleted</v-icon></v-avatar
            >
            <div>
        ...
        <v-card rounded="lg" elevation="0" border>
          <v-card-text class="d-flex align-center ga-3">
            <v-avatar color="green-darken-2" variant="tonal" size="44"
              ><v-icon>mdi-tag-multiple</v-icon></v-avatar
            >
            <div>
        ...
        <v-card rounded="lg" elevation="0" border>
          <v-card-text class="d-flex align-center ga-3">
            <v-avatar color="orange-darken-2" variant="tonal" size="44"
              ><v-icon>mdi-calendar-today</v-icon></v-avatar
            >
            <div>
        ...
        <v-card rounded="lg" elevation="0" border>
          <v-card-text class="d-flex align-center ga-3">
            <v-avatar color="info" variant="tonal" size="44"
              ><v-icon>mdi-calendar-star</v-icon></v-avatar
            >
            <div>
    ...
    <v-card v-if="!loading && rows.length === 0" rounded="lg" elevation="0" border class="mb-6">
      <v-card-text class="text-center pa-12">
        ...
        <v-btn color="primary" class="mt-3" prepend-icon="mdi-plus" @click="openAddDialog"
          >Add First Row</v-btn
        >
      </v-card-text>
    </v-card>
      ...
      <v-card rounded="lg">
        <v-card-text class="pa-4 text-center">
          ...
          <p class="text-medium-emphasis">
            Are you sure you want to delete <strong>{{ selectedRow?.component }}</strong
...
// AFTER — handle empty table gracefully
async function fetchSignatories() {
  const { data } = await supabase.from('pm_signatories').select('*').limit(1).maybeSingle()
  if (data) signatories.value = data
  // if no row yet, signatories.value stays null — button is disabled below
}
